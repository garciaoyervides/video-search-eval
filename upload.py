...
import clip
import data
from databasechroma import delete_all_segments_from_video,add_segment, write_to_log
...
def process_video_frames(video_name):
    ...
    while(True):
        ret,frame = cam.read()
        if ret:
            outname = f'./tmp/img/frame_{str(i).zfill(8)}.jpg'
    ...
    for im in os.listdir(f'./tmp/img/'):
        if os.path.isfile(f'./tmp/img/{im}'):
            ...
            frame_no = int(im[im.find("_")+1:im.find(".")])
            time = int((frame_no*100)/video_fps)
...
def process_video_keyframes(video_name):
    ...
    if i_frames:
        ...
        for frame_no in i_frames:
            ...
            _, frame = cam.read()
            frame_num = str(int(frame_no)).zfill(8)
            outname = f'./tmp/img/frame_{frame_num}.jpg'
            cv2.imwrite(outname, frame)
        cam.release()
    #cv2.destroyAllWindows()
    images = []
...
def get_msb_segmentation(file_name):
  try:
    msb = f"./msb/{file_name.replace('.mp4', '.msb')}"
    with open(msb, 'r', encoding='latin-1') as csvfile:
      csvreader = csv.reader(csvfile, delimiter=' ')
      return list(csvreader)[2:]
  except:
    print(f"{file_name} not available")
    return None

def process_marked_segmentation(image_features,time,video_name):
    ...
    segmentation = []
    msb_segmentation = get_msb_segmentation(video_name)
    for i,seg in enumerate(msb_segmentation):
        start_frame = int(seg[0])
        end_frame = int(seg[1])
        start_time = time[start_frame]
        end_time = time[end_frame]
        scene_features = torch.mean(image_features_torch[start_frame:end_frame],0)
        scene_features = torch.reshape(scene_features, (1,-1))
        segmentation.append({
                'video name':video_name,
                'scene':i,
                'start':start_time,
                'end':end_time,
                'features':scene_features.tolist()[0]
            })

    return segmentation

def save_segments(video_name, segmentation):
    delete_all_segments_from_video(video_name)
    for segment in segmentation:
        segment_id = f"{video_name} {str(segment['scene']).zfill(6)}"
        metadata = {
            "video name":video_name,
            "scene":segment['scene'],
            "start":segment['start'],
        ...
        add_segment(segment_id, segment['features'], metadata)
...
def process_video_to_db(video_name):
    ...
    #Get segmentation
    segmentation = process_marked_segmentation(image_features,time,video_name)
    #Save to DB
